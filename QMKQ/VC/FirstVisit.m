...
@interface FirstVisit () <UITableViewDelegate,UITableViewDataSource,QBImagePickerControllerDelegate,UITextViewDelegate,UITextFieldDelegate>
{
...
@property (nonatomic, assign) NSInteger mouth_hygiene;//口腔卫生
@property (nonatomic, retain) UITextField * yaZhouYan;//亚洲状态描述
...
@property (nonatomic, retain) UITextField*wear_time;//wear_time
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    CaseHeaderView*view=[tableView dequeueReusableHeaderFooterViewWithIdentifier:@"firstVisitHeader"];
    if (!view) {
        view = [[CaseHeaderView alloc]initWithReuseIdentifier:@"firstVisitHeader"];
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else if (section == 1) number = 6;
    else if (section == 2) number = 20;
    return number;
}
...
//        cel.other.text = @"| 女 | 2005.8.7";
        ...
        if (row == 0) {
            ...
            cel.leftLabel.text = @"口腔卫生";
            cel.RightLabel.text = @"";
            [cel.first setTitle:@"好" forState:0];
            [cel.second setTitle:@"中" forState:0];
            [cel.third setTitle:@"差" forState:0];
            [Tools button:cel.first removeAllActionsFromTarget:self];
            [Tools button:cel.first andOthersButtonAddTarget:self action:@selector(kouqiangweisheng:)];
            cell = cel;
        }else if (row == 1) {
            identifierString = @"PeriodontalCell";
            PeriodontalCell*cel = [tableView dequeueReusableCellWithIdentifier:identifierString];
            cel.textfield.delegate=self;
            cel.textfield.layer.borderColor = LineColor.CGColor;
            cel.textfield.layer.borderWidth = 0.5f;
            self.yaZhouYan=cel.textfield;
            cel.left.text = @"牙周";
            cell = cel;
        }else if (row == 2) {
...
- (void)kouqiangweisheng:(DLRadioButton*)bt
{
    if (bt.selectedButton) {
        CGPoint p = [bt convertPoint:CGPointZero toView:self.tableView];
        NSIndexPath*indexPath = [self.tableView indexPathForRowAtPoint:p];
        ...
        self.mouth_hygiene=t;
    }else {
        self.mouth_hygiene=0;
    }
}
...
- (void)qb_imagePickerController:(QBImagePickerController *)imagePickerController didSelectAssets:(NSArray *)assets
{
    ...
    [self.imageDic setObject:SafeValue(image, [UIImage imageNamed:@"zp"]) forKey:self.imageViewIndex];
...
- (IBAction)tijiaoAction:(UIButton*)sender {
    ...
    [dictionary setObject:[NSString stringWithFormat:@"%d",(int)self.mouth_hygiene] forKey:@"mouth_hygiene"];
    [dictionary setObject:SafeValue(self.yaZhouYan.text, @"") forKey:@"yaZhouYan"];
